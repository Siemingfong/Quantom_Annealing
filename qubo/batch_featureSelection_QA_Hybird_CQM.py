...
for i in range(0, 6):
    ...
    # 固定保留前 100000 行
    df = df.iloc[:10000, :]  # 固定保留前 100000 行
    ...
    qpu_advantage = DWaveSampler(solver={'chip_id': 'hybrid_constrained_quadratic_model_version1p'})
    ...
    # 計算總時間並打印出來
    quantum_computation_time = end_time - start_time
    print(f"Class {i} Quantum Computation Time: {quantum_computation_time:.4f} seconds")  # 四捨五入到小數點四位數
